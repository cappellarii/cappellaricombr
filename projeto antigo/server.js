...
app.get("/presence", requireAuth, (_req, res) => {
    ...
    const users = Array.from(onlineUsers.keys()).sort((a, b) => a.localeCompare(b));
...
app.post("/messages", requireAuth, (req, res) => {
    upload.single("file")(req, res, (uploadErr) => {
        ...
        return resolveChannel(channelId, (channelErr) => {
            ...
            messagesDb.run(
                "INSERT INTO messages (name, text, file_url, file_name, file_mime, file_size, channel_id) VALUES (?, ?, ?, ?, ?, ?, ?)",
                [cleanName, text || null, fileUrl, fileName, fileMime, fileSize, channelId],
